...
function createImageFromBytes(bytes: Uint8Array): ImagePaint {
...
async function addPointerToCanvas(imageBytes: Uint8Array) {
  try {
...
async function attachSoundToElement(nodeId: string, soundBytes: Uint8Array) {
  try {
...
// Создание анимации
function createAnimation(nodeId: string, animationType: string) {
  try {
    const node = figma.getNodeById(nodeId) as SceneNode;
    if (!node) {
      figma.notify("Элемент не найден");
...
function createBounceAnimation(node: SceneNode) {
...
function createPulseAnimation(node: SceneNode) {
...
function createShakeAnimation(node: SceneNode) {
...
function createEmotionWheelAnimation(node: SceneNode) {
